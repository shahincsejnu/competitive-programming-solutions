...
<head>
    ...
    <meta name="X-Csrf-Token" content="891c2a4193118eb17377957be410faf2"/>
    ...
    <meta property="og:title" content="Submission #81470159 - Codeforces"/>
    ...
    <meta name="cc" content="26c6a571149d115cfd1f4f9da0aa18dad714b438"/>
    
    
    <meta name="utc_offset" content="+03:00"/>
    <meta name="verify-reformal" content="f56f99fd7e087fb6ccb48ef2" />
    <title>Submission #81470159 - Codeforces</title>
...
<body class=" "><span style='display:none;' class='csrf-token' data-csrf='891c2a4193118eb17377957be410faf2'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F463%2Fsubmission%2F81470159">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='891c2a4193118eb17377957be410faf2'/>
...
<a href="/profile/shahincsejnu" title="Newbie shahincsejnu" class="rated-user user-gray">shahincsejnu</a>    </td>
    ...
    <div class="roundbox SubmissionDetailsFrameRoundBox-81470159" style="margin-top:2em;font-size:1.1rem;">
            <div class="roundbox-lt">&nbsp;</div>
            <div class="roundbox-rt">&nbsp;</div>
...
int n;
lli arr[2005][2005], mx_val[2005][2005], took[2005][2005];

void get_total(int x, int y, int flag)
{
    lli val;
    
    if(flag == 1){
        val = 0;
        for(int i = x, j = y; i <= n and j <= n; i++, j++) val += arr[i][j];
        for(int i = x, j = y; i <= n and j <= n; i++, j++) {mx_val[i][j] = val; took[i][j] = 1;}

        val = 0;
        for(int i = x, j = y; i <= n and j >= 1; i++, j--) val += arr[i][j];
        for(int i = x, j = y; i <= n and j >= 1; i++, j--){
            if(took[i][j] == -1) {mx_val[i][j] = val; took[i][j] = 1; continue;}
            mx_val[i][j] += (val-arr[i][j]);
        }
    }
    else{
        val = 0;
        for(int i = x, j = y; i >= 1 and j <= n; i--, j++) val += arr[i][j];
        for(int i = x, j = y; i >= 1 and j <= n; i--, j++){
            if(took[i][j] == -1) {mx_val[i][j] = val; took[i][j] = 1; continue;}
            mx_val[i][j] += (val-arr[i][j]);
        }

        val = 0;
        for(int i = x, j = y; i >= 1 and j >= 1; i--, j--) val += arr[i][j];
        for(int i = x, j = y; i >= 1 and j >= 1; i--, j--){
            if(took[i][j] == -1) {mx_val[i][j] = val; took[i][j] = 1; continue;}
            mx_val[i][j] += (val-arr[i][j]);
        }
    }
...
void solve()
{
    ...
    for(int i = 1; i <= n; i++) get_total(1, i, 1);
    
    for(int i = 1; i <= n; i++){
        if(took[n][i] == -1) get_total(n, i, 2);
    }
    
    int x1 = -1, y1 = -1, x2 = -1, y2 = -1, start;
    lli mx1 = -1, mx2 = -1;
