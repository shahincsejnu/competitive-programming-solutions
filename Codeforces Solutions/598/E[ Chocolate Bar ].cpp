...
<head>
    ...
    <meta name="X-Csrf-Token" content="ff6bba9bb107c77b6cd35f2186afdb99"/>
    ...
    <meta property="og:title" content="Submission #64636707 - Codeforces"/>
    ...
    <title>Submission #64636707 - Codeforces</title>
...
<body class=" "><span style='display:none;' class='csrf-token' data-csrf='ff6bba9bb107c77b6cd35f2186afdb99'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F598%2Fsubmission%2F64636707">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='ff6bba9bb107c77b6cd35f2186afdb99'/>
...
<a href="/profile/shahincsejnu" title="Newbie shahincsejnu" class="rated-user user-gray">shahincsejnu</a>    </td>
    <td>
        <a title="E - Chocolate Bar" href="/contest/598/problem/E">598E</a>
    ...
    <div class="roundbox SubmissionDetailsFrameRoundBox-64636707" style="margin-top:2em;font-size:1.1rem;">
            <div class="roundbox-lt">&nbsp;</div>
            <div class="roundbox-rt">&nbsp;</div>
...
const int inf = 1e9 + 5;
int dp[35][35][55], n, m, k;

int func(int row, int col, int baki)
{
	if(row*col == baki) return dp[row][col][baki] = 0;
	if(baki == 0) return 0;
	
	if(dp[row][col][baki] != -1) return dp[row][col][baki];
	
	int p = inf;
	
	for(int i = 1; i < row; i++){
		if((i*col) <= baki) p = min(p, func(row-i, col, baki - i*col) + col*col);
		else{
			int q = min(func(i, col, baki), func(row-i, col, baki)) + col*col;
			p = min(p, q);
		}
	}
	
	for(int i = 1; i < col; i++){
		if((i*row) <= baki) p = min(p, func(row, col - i, baki - i*row) + row*row);
		else{
			int q = min(func(row, i, baki), func(row, col-i, baki)) + row*row;
			p = min(p, q);
		}
	}
	
	return dp[row][col][baki] = p;
}
